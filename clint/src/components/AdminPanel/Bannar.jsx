...
const Bannar = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.status === 200) {
        // Item deleted successfully, update the table data
        fetchBannerData(); // Assuming you have a function to fetch the updated data
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <img
          src={`http://localhost:8000/${record.img}`}
          alt={record.img}
